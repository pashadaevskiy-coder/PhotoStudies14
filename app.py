...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:////tmp/photo_studios.db')
    ...
    try:
        with app.app_context():
            ...
            if count == 0:
                ...
                db.session.commit()
                return jsonify({'success': True, 'message': f'База данных инициализирована, добавлено {len(test_bookings)} тестовых записей'})
            
            return jsonify({'success': True, 'message': f'База данных уже содержит {count} записей'})
